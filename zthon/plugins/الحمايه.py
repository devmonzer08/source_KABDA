...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if custompmpermit is not None:
        ...
        USER_BOT_NO_WARN = f"""ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗸𝗲𝗯𝗱𝗮  **- الـرد التلقـائي 〽️**
**•─────────────────•**
...
async def on_new_private_message(event):
    ...
    if event.chat_id == 6655082279 or event.chat_id == 0000000000 or event.chat_id == 0000000000 or event.chat_id == 00000000000:
...
async def approve_p_m(event):
    ...
    await edit_or_reply(
        ...
        caption="𝗸𝗲𝗯𝗱𝗮 المسمـوح لهـم ( المقبوليـن )**\n\n**- سـورس ريفز** 𝗸𝗲𝗯𝗱𝗮 ",
    )
